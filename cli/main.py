...
ERROR_LOG_FILE = ERROR_LOG_DIR / "cli_error.log"
...
def _excepthook(exc_type, exc, tb) -> None:
    try:
        ...
        with ERROR_LOG_FILE.open("a", encoding="utf-8") as f:
            ts = datetime.datetime.now().isoformat()
            f.write(f"\n{'='*80}\n")
            f.write(f"[{ts}] {exc_type.__name__}: {exc}\n")
...
def get_user_selections():
    ...
    # Step 3: Analysts Team (all analysts run in parallel in current architecture)
    console.print(
        create_question_box(
            "Step 3: Analysts Team",
            "All analysts (Market, News, Social, Fundamentals) will run in parallel"
        )
    )
    selected_analysts = [AnalystType.MARKET, AnalystType.SOCIAL, AnalystType.NEWS, AnalystType.FUNDAMENTALS]
    console.print(
        f"[green]All analysts will be used:[/green] {', '.join(analyst.value for analyst in selected_analysts)}"
    )
...
def run_analysis():
    ...
    with Live(layout, refresh_per_second=4) as live:
        ...
        message_buffer.add_message(
            "System",
            f"All analysts running in parallel: {', '.join(analyst.value for analyst in selections['analysts'])}",
        )
        update_display(layout)
        ...
        # Update all analysts to in_progress (they execute in parallel)
        ...
        for chunk in graph.graph.stream(init_agent_state, **args):
            if len(chunk["messages"]) > 0:
                ...
                # Analyst Team Reports (all run in parallel)
                ...
                if "fundamentals_report" in chunk and chunk["fundamentals_report"]:
                    ...
                    # When all analysts complete, set research team to in_progress
                    all_analysts_done = all([
                        message_buffer.agent_status.get("Market Analyst") == "completed",
                        message_buffer.agent_status.get("Social Analyst") == "completed",
                        message_buffer.agent_status.get("News Analyst") == "completed",
                        message_buffer.agent_status.get("Fundamentals Analyst") == "completed",
                    ])
